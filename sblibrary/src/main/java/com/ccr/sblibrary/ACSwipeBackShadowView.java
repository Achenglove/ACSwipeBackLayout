 ...
 * 左侧阴影控件
 */
class ACSwipeBackShadowView extends FrameLayout {
    private static final String TAG = ACSwipeBackShadowView.class.getSimpleName();
    ...
    ACSwipeBackShadowView(Activity activity) {
        super(activity);
        mActivity = activity;
    ...
    void bindPreActivity() {
        ...
        if (mPreActivity == null) {
            Activity preActivity = ACSwipeBackManager.getInstance().getPenultimateActivity(mActivity);
            if (preActivity != null) {
    ...
    private Bitmap getBitmap(View view) {
        ...
        Bitmap bitmap = Bitmap.createBitmap(view.getDrawingCache(), 0, 0,
                ACUIUtil.getRealScreenWidth(mActivity),
                ACUIUtil.getRealScreenHeight(mActivity) - ACUIUtil.getNavigationBarHeight(mActivity));
    ...
    private boolean containsProblemView(ViewGroup viewGroup) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (ACSwipeBackManager.getInstance().isProblemView(childView)) {
                return true;
    ...
    private void onPanelSlide(Activity currentActivity, float slideOffset) {
        try {
            Activity preActivity = ACSwipeBackManager.getInstance().getPenultimateActivity(currentActivity);
            if (preActivity != null) {
