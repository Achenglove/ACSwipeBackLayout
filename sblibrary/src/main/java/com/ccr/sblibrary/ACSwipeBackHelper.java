 ...
 * @Date: 2017-12-01 09:32
 * @Version: V1.0 <描述当前版本功能>
 * 在此写用途
 * 滑动返回帮助类
 */
public class ACSwipeBackHelper {
    private Activity mActivity;
    private Delegate mDelegate;
    private ACSwipeBackLayout mSwipeBackLayout;
    ...
    public static void init(Application application, List<Class<? extends View>> problemViewClassList) {
        ACSwipeBackManager.getInstance().init(application, problemViewClassList);
    }
     ...
     * @param delegate
     */
    public ACSwipeBackHelper(Activity activity, Delegate delegate) {
        mActivity = activity;
        mDelegate = delegate;
    ...
    private void initSwipeBackFinish() {
        if (mDelegate.isSupportSwipeBack()) {
            mSwipeBackLayout = new ACSwipeBackLayout(mActivity);
            mSwipeBackLayout.attachToActivity(mActivity);
            mSwipeBackLayout.setPanelSlideListener(new ACSwipeBackLayout.PanelSlideListener() {
                @Override
                public void onPanelSlide(View panel, float slideOffset) {
                    // 开始滑动返回时关闭软键盘
                    if (slideOffset < 0.03) {
                        ACKeyboardUtil.closeKeyboard(mActivity);
                    }
     ...
     * @return
     */
    public ACSwipeBackHelper setSwipeBackEnable(boolean swipeBackEnable) {
     ...
     * @return
     */
    public ACSwipeBackHelper setIsOnlyTrackingLeftEdge(boolean isOnlyTrackingLeftEdge) {
        if (mSwipeBackLayout != null) {
     ...
     * @return
     */
    public ACSwipeBackHelper setIsWeChatStyle(boolean isWeChatStyle) {
        if (mSwipeBackLayout != null) {
     ...
     * @return
     */
    public ACSwipeBackHelper setShadowResId(@DrawableRes int shadowResId) {
        if (mSwipeBackLayout != null) {
     ...
     * @return
     */
    public ACSwipeBackHelper setIsNeedShowShadow(boolean isNeedShowShadow) {
        if (mSwipeBackLayout != null) {
     ...
     * @return
     */
    public ACSwipeBackHelper setIsShadowAlphaGradient(boolean isShadowAlphaGradient) {
        if (mSwipeBackLayout != null) {
     ...
     * @param threshold
     */
    public ACSwipeBackHelper setSwipeBackThreshold(@FloatRange(from = 0.0f, to = 1.0f) float threshold) {
        if (mSwipeBackLayout != null) {
     ...
     * @param overlap
     */
    public ACSwipeBackHelper setIsNavigationBarOverlap(boolean overlap) {
        if (mSwipeBackLayout != null) {
